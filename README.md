...
PixelLift AI 是一个面向电商和营销场景的 AI 产品图高清放大工作台。项目基于 Next.js + FastAPI 搭建端到端图片处理流程，支持图片上传、异步任务处理、Real-ESRGAN 高清放大、候选结果对比、批量处理、人工反馈和评测报告导出。

它的目标不是做一个简单的图片处理 Demo，而是验证一条可落地的产品图增强链路：从低清商品图输入，到 2x/4x 高清候选结果生成，再到质量评估和后续模型优化数据沉淀。

## 项目亮点

- **完整产品链路**：上传、任务创建、异步推理、结果对比、反馈评测和导出已跑通。
- **真实模型接入**：支持 Real-ESRGAN ncnn-vulkan，本地无 GPU 时可切换 stub adapter 开发。
- **批量处理能力**：支持多图上传、批量结果下载、评测报告和风险样本清单导出。
- **工程可扩展性**：通过统一 model adapter 预留 SwinIR、HAT、DiffBIR、SUPIR 等模型接入空间。
